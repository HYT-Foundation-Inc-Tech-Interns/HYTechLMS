  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!isVerificationExempt && !credential.user.emailVerified) {
        // Send a fresh link (best-effort) and hand off to the shared
        // /verify-email screen. Keep the (unverified) session alive so that
        // page can resend and poll without asking for the password again.
        let emailSent = true;
        try {
        ...
        addToast(
          emailSent
            ? `Please verify your email before signing in. We sent a new link to ${normalizedEmail}.`
            : 'Please verify your email before signing in. You can resend the link on the next screen.',
          'warning'
        );
