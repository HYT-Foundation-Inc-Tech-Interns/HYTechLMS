...
const SupervisorNavbar = ({ title, subtitle }) => {
  const [showDropdown, setShowDropdown] = useState(false);
  const dropdownRef = useRef(null);
  const navigate = useNavigate();
  const { avatar } = useProfileAvatar('supervisor');
  ...
  return (
    ...
    <header className="text-white h-16 px-3 sm:px-4 md:px-6 flex items-center justify-between shadow-lg relative z-50 flex-shrink-0" style={{ backgroundColor: '#0B005C' }}>
      ...
      <div className="flex items-center gap-2 sm:gap-4 pl-2">
        {/* Notifications */}
        <NotificationDropdown role="supervisor" />
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          >
            {avatar ? (
              <img src={avatar} alt="Supervisor profile" className="w-9 h-9 rounded-full object-cover border border-white/20" />
            ) : (
              <div className="w-9 h-9 bg-indigo-500 rounded-full flex items-center justify-center font-semibold text-sm shadow-lg">
                {initials}
              </div>
            )}
          </button>
